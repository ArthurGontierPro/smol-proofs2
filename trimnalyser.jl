...
=#
    ...
    # Pre-allocated singleton link vectors for systemlink entries that are never modified after creation.
    # Sharing is safe because all code that mutates existing systemlink entries (readsubproof, getcone! sort)
    # is dead (lparse undefined / red..begin..end blocks never generated by Glasgow solver).
    const _LINK_RUP  = Int[-1]   # rup
